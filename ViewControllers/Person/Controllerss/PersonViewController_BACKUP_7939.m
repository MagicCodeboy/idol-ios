...
//        NSLog(@"关注数量：%@",StringWithFormat(model.followCount))
        ...
        if (ISNULL(model.avatar)||[model.avatar isEqualToString:@""]) {
            cell.headerImg.image=IMAGE(@"morentouxiang");
        }
        else{
        [cell.headerImg sd_setImageWithURL:[NSURL URLWithString:model.avatar] placeholderImage:IMAGE(@"morentouxiang")];
...
=======
                                if ([imageURL isKindOfClass:[NSNull class]]|| [StringWithFormat(imageURL) isEqualToString:@"(null)"]||[model.bigAvatar isEqualToString:@""]) {
                                    
                                    cell.bigImageView.image=[UIImage  imageNamed:@"gerenzhongxin－morenbeijing"];
//                                    quanjuxinglogo
                                    SDImageCache *cache=[SDImageCache sharedImageCache];
                                    [cache storeImage:IMAGE(@"gerenzhongxin－morenbeijing") forKey:@"userHeaderImgView" toDisk:YES];
                                    
                                    
                                }    else
>>>>>>> f3b2ef16e8b4f88d78b195b04fa967f1c69c94c2
                                {
                                    UIImage *images=[image blurredImageWithRadius:5];

                                    cell.bigImageView.image=images;
