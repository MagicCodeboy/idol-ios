...
-(void)configUI
{
    ...
    if (_detailView == nil)
    {
        ...
        [_detailView.detailComeBack addActionHandler:^(NSInteger tag) {
            ...
            self.tapiamgeView.userInteractionEnabled = YES;
        }];
        //购买按钮的点击事件
    ...
    if (_orderListView == nil)
    {
        ...
        [_orderListView.changeAddress addActionHandler:^(NSInteger tag) {
            weakSelf.orderListView.hidden = YES;
            weakSelf.addRessView.receiveName.text = weakSelf.orderListView.orderReceiveName.text;
            weakSelf.addRessView.receivePhoneNumber.text = weakSelf.orderListView.orderReceivePhone.text;
            weakSelf.addRessView.detailAddressLable.text = weakSelf.orderListView.orderAddress.text;
            weakSelf.addRessView.detailAddressLable.textColor = UIColorFromRGB(0x333333);
        ...
        [_addRessView.addressSave addActionHandler:^(NSInteger tag) {
            ...
            if (_addRessView.receiveName.text.length>0  )
            {
                if ( _addRessView.receivePhoneNumber.text.length>0)
                {
                    ...
                    {
                        //点击保存按钮的时候 返回到上一个页面 展示保存的地址的信息
                        self.orderListView.orderReceiveName.text = self.addRessView.receiveName.text;
                        self.orderListView.orderReceivePhone.text = self.addRessView.receivePhoneNumber.text;
                        self.orderListView.orderAddress.text = self.addRessView.detailAddress.text;
                        weakSelf.orderListView.hidden = NO;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.tapiamgeView.userInteractionEnabled = NO;
}
...
- (void)keyboardWillChange:(NSNotification *)note
{
    ...
    [UIView animateWithDuration:duration animations:^{
        self.addRessView.frame = CGRectMake(0, moveHight, SCREEN_WIDTH, self.addRessView.frame.size.height);
...
{
    if (textField == self.addRessView.receiveName){
        self.addRessView.receiveName.text = @"";
        self.addRessView.receiveName.placeholder = @"请填写";
        
    }
    else
    {
        self.addRessView.receivePhoneNumber.text = @"";
        self.addRessView.receivePhoneNumber.placeholder = @"请填写";
    }
}
...
{
    self.addRessView.detailAddressLable.hidden = YES;
    self.addRessView.detailAddress.text = @"";
    self.addRessView.frame =  self.addRessView.frame = CGRectMake(0, moveHight - 20, SCREEN_WIDTH, self.addRessView.frame.size.height + 20);
}
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([text isEqualToString:@"\n"]) {
        ...
        self.addRessView.frame = CGRectMake(0, SCREEN_HEIGHT -196, SCREEN_WIDTH, 196);
//        self.addRessView.detailAddressLable.hidden = NO;
//        self.addRessView.detailAddressLable.textColor = UIColorFromRGB(0x333333);
//        self.addRessView.detailAddressLable.text = self.addRessView.detailAddress.text;
        [self.addRessView.detailAddress resignFirstResponder];
        return NO;
    }
    if ([text isEqual:@""]) {
...
//
//        [UIView animateWithDuration:0.3 animations:^{
//            self.addRessView.frame = CGRectMake(0, moveHight-height+40, SCREEN_WIDTH, self.addRessView.frame.size.height+height-40);
//        }];
//    } completion:nil];
