...
>>>>>>> 666bc98c1fe0eadc2a2fae62165fd2d03c69fb42
...
-(void)awakeFromNib
{
    ...
    custom.hidden = YES;
    custom.sendMessageTf.clearsOnBeginEditing = YES;
...
-(void)ShowonlineList
{
    ...
    //SCREEN_WIDTH
    _ListCollectionView=[[UICollectionView alloc]initWithFrame:CGRectMake(165, 26, SCREEN_WIDTH - 150-12-30-15, 41) collectionViewLayout:flowOut];
   ...
   // _ListCollectionView.layer.cornerRadius=self.ListCollectionView.frame.size.height/2;
    ...
    [_ListCollectionView registerNib:[UINib nibWithNibName:@"residentViewCell" bundle:nil] forCellWithReuseIdentifier:VIEWERS];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (collectionView==_ListCollectionView)
    {
        ...
        residentViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:VIEWERS forIndexPath:indexPath];
...
//            //[cell.residentImage sd_setImageWithURL:[NSURL URLWithString:model.picUrl]];
            [cell.residentImage sd_setImageWithURL:[NSURL URLWithString:@"http:\/\/joustar.img-cn-beijing.aliyuncs.com\/image\/user\/2016\/07\/01\/160701143852649113.jpg@!face"]];
...
-(void)onSendgiftItem:(Customize *)xiaoxi
{
    ...
    if (xiaoxi.gift_type == 1 || xiaoxi.gift_type == 2) {
    ...
    if (xiaoxi.gift_type == 3 || xiaoxi.gift_type == 4) {
        [self ShowBigGifts];
...
-(void)ShowBigGifts
{
   ...
   [images addObject:[UIImage imageNamed:[NSString stringWithFormat:@"feiwen%02d.png",i]]];
    ...
    BigGift = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_WIDTH)];
...
-(void)initHeadView{
    ...
    _HeadImage.layer.cornerRadius = self.HeadImage.height/2; //宽度／2
    _HeadImage.layer.masksToBounds = YES;

    [self.HeadGuanzhu.layer setCornerRadius:self.HeadGuanzhu.height/2];
    [self.HeadGuanzhu.layer setBorderWidth:1.0];
...
- (void)sendMsgBtnClick
{
    ...
    if (custom.sendMessageTf.text) {
        NIMMessage *message = [[NIMMessage alloc] init];
        message.text    = custom.sendMessageTf.text;
        //构造会话
        NIMSession *session = [NIMSession session:self.pxyViewersModel.userLiveBase.roomId type:NIMSessionTypeChatroom];
        //发送消息
...
//            EXmessage.roomAvatar =@"ncxfj.png";
           ...
           // [self updata:message.text];
            ...
            NIMNotificationObject *bb = (NIMNotificationObject *)message.messageObject;
            NIMChatroomNotificationContent  *bb2 =(NIMChatroomNotificationContent *)bb.content;
            
            NSLog(@"-------bb2.notifyExt------%@-----------",bb2.notifyExt);
            
            NSData *data = [bb2.notifyExt dataUsingEncoding:NSUTF8StringEncoding];
            if (data) {
                NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
                NSString *head_url = dict[@"head_url"];
                NSLog(@"---------head_url----%@-----------",head_url);
                
//                
//                PXYHeadViewModel *model=[[PXYHeadViewModel alloc]init];
//                model.picUrl =@"http:\/\/joustar.img-cn-beijing.aliyuncs.com\/image\/user\/2016\/07\/01\/160701143852649113.jpg@!face";
//                model.Useid = @"12201";
//                if (_headarray.count <20) {
//                    [_headarray insertObject:model atIndex:1];
//                    
//                }else
//                {
