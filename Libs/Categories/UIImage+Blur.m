...
- (UIImage *)lightImage
{
    ...
    return [self blurredImageWithSize:CGSizeMake(20, 20) tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
+(UIImage *)makeThumbnailFromImage:(UIImage *)srcImage scale:(double)imageScale {
    ...
    NSData *imageData = UIImageJPEGRepresentation(thumbnail, 1.0); //PNG格式
    ...
    if (imageData.length > 1024*1024) {//1M以及以上
        ...
        imageData = UIImageJPEGRepresentation(thumbnail, 0.5);
    }else if (imageData.length > 200*1024) {//0.25M-0.5M
        imageData = UIImageJPEGRepresentation(thumbnail, 0.9);
    }
...
+(UIImage *)makeThumbnailFromImage:(UIImage *)srcImage {
    ...
    CGFloat width=SCREEN_SHIJI_WIDTH;
    if(srcImage.size.width>width || srcImage.size.height>width)
    {
        if(srcImage.size.width/width>srcImage.size.height/width)
        {
            imageSize = CGSizeMake(width, srcImage.size.height *width/srcImage.size.width);
        }else{
            imageSize = CGSizeMake(srcImage.size.width * width/srcImage.size.height,width);
        }
    }
...
+ (NSData *)compressToJPEGFormatDataWithFactor:(CGFloat)factor maxFileSize:(u_int64_t)fileSize image:(UIImage *)image
{
    ...
    while (fabs(maxFactor-minFactor) > compressionFactor)
    {
        @autoreleasepool
        {
            ...
            tempImageData = UIImageJPEGRepresentation(image, midFactor);
