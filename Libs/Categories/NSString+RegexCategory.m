...
//验证数字
-(BOOL)isNumber{
    NSString * numberString = @"^[0-9]*$";
    BOOL ret = [self isValidateByRegex:numberString];
    
    return ret;
}
-(BOOL)isvalidNumberContainsPoint{
    NSString * numberString = @"((^[-]?([1-9]\\d{0,6}))|^0)(\\.\\d{1,2})?$|(^[-]0\\.\\d{1,2}$)";
    BOOL ret = [self isValidateByRegex:numberString];
    
    return ret;
}
-(BOOL)isvalidBountyContainsPoint {
    NSString * numberString = @"^(([1-7]\\d?)+(\\.[0-9]{1})?|80|80.0)$";
    BOOL ret = [self isValidateByRegex:numberString];
...
- (BOOL)isMobileNumber{
     ...
     */
    NSString *mobileNoRegex = @"^1\\d{10}$";//除4以外的所有个位整数，不能使用[^4,\\d]匹配，这里是否iOS Bug?
...
- (BOOL)isValidWithMinLenth:(NSInteger)minLenth
             ...
             containChinese:(BOOL)containChinese containOtherCharacter:(BOOL)Character
        firstCannotBeDigtal:(BOOL)firstCannotBeDigtal;
{
    ...
    NSString *biaodianss = Character ? @"(?=.*[!@#$%^&*])" : @"";

    

    NSString *regex = [NSString stringWithFormat:@"%@[%@A-Za-z0-9_%@]{%d,%d}", first, hanzi, biaodianss,(int)(minLenth-1), (int)(maxLenth-1)];
    return [self isValidateByRegex:regex];
}
