...
//        _scrollView.alwaysBounceHorizontal = YES;
        ...
        //点击手势
        //长按手势
        UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(event_tapgestureMethod:)];
...
- (Y_StockChartRightYView *)priceView
{
    if(!_priceView)
    {
        ...
        _priceView.isShouldShowTheTwoLabel = true;
...
- (Y_StockChartRightYView *)volumeView
{
    if(!_volumeView)
    {
        ...
        _priceView.isShouldShowTheTwoLabel = NO;
...
- (void)setTargetLineStatus:(Y_StockChartTargetLineStatus)targetLineStatus
{
    ...
    if(targetLineStatus < 103)
    {
        ...
        NSLog(@"scrollview的偏移量%f和scrollview的contentsize%f",self.scrollView.contentOffset.x,self.scrollView.contentSize.width);
//        CGPoint contentOffsetPoint = self.scrollView.contentOffset;
//        CGRect frame = self.scrollView.frame;
//        if (contentOffsetPoint.x == self.scrollView.contentSize.width - frame.size.width) {
//            [self reDraw];
//        }
...
-(void)event_tapgestureMethod:(UITapGestureRecognizer *)tapGesture{
...
{
    self.volumeView.isShouldShowTheTwoLabel = NO;
...
{
    self.priceView.isShouldShowTheTwoLabel = NO;
...
{
    self.accessoryView.isShouldShowTheTwoLabel = NO;
...
//    }
//
//    NSLog(@"这是偏移量哈哈哈哈哈  %f-----%f=====%f",scrollView.contentSize.width,scrollView.contentOffset.x,self.kLineMainView.width);
    CGPoint contentOffsetPoint = self.scrollView.contentOffset;
    CGRect frame = self.scrollView.frame;
    float theOffset = 1.0;
    if (contentOffsetPoint.x == self.scrollView.contentSize.width - frame.size.width) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"TheExchangeNotification" object:nil userInfo:@{@"theOffset":[NSNumber numberWithFloat:theOffset]}];
}
