...
- (UIView *)indicatorView
{
    if(!_indicatorView)
    {
        ...
        _indicatorView.frame = CGRectMake(0, -50, SCREEN_WIDTH, 50);
        _indicatorView.backgroundColor = [UIColor assistBackgroundColor];
        
        self.isHaveShowThelist = NO;
        
        NSArray *titleArr = @[@"MACD",@"KDJ",@"关闭",@"MA",@"EMA",@"BOLL",@"关闭"];
        CGFloat buttonW = (SCREEN_WIDTH - (15 * titleArr.count + 15)) / titleArr.count;
        UIButton *preBtn = nil;
        for (int i = 0; i < titleArr.count; i++) {
            UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
            if (preBtn) {
                btn.frame = CGRectMake(preBtn.frame.origin.x + 15, 0, buttonW, 50);
            } else {
                btn.frame = CGRectMake((buttonW + 15) * i + 15, 0, buttonW, 50);
            }
            NSLog(@"theButtonX is%f",btn.frame.origin.x);
            ...
            btn.tag = Y_StockChartSegmentStartTag + 100 + i;
...
//                btn.frame = CGRectMake(preBtn.frame.origin.x + 15, 0, buttonW, 50);
//            } else {
//                btn.frame = CGRectMake((buttonW + 15) * idx + 15, 0, buttonW, 50);
//            }
//            [btn setTitleColor:[UIColor mainTextColor] forState:UIControlStateNormal];
//            [btn setTitleColor:[UIColor ma30Color] forState:UIControlStateSelected];
//            btn.titleLabel.font = [UIFont systemFontOfSize:13];
...
- (void)setItems:(NSArray *)items
{
    ...
    UIButton *preBtn = nil;
    CGFloat buttonW = (SCREEN_WIDTH - (15 * count + 15)) / count;
    for (int i = 0; i < count; i++) {
        UIButton *btn = [self private_createButtonWithTitle:items[i] tag:Y_StockChartSegmentStartTag+i];
        if (preBtn) {
            btn.frame = CGRectMake(preBtn.frame.origin.x + 15, 0, buttonW, 50);
        } else {
            btn.frame = CGRectMake((buttonW + 15) * i + 15, 0, buttonW, 50);
