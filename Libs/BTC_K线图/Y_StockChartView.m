...
- (Y_StockChartSegmentView *)segmentView
{
    if(!_segmentView)
    {
        ...
        _segmentView.backgroundColor = RGB(42, 20, 75);
...
- (void)y_StockChartSegmentView:(Y_StockChartSegmentView *)segmentView clickSegmentButtonIndex:(NSInteger)index
{
    ...
    if (index == 105) {
        self.segmentView.isHaveShowThelist = NO;
...
//        }
        ...
        if(self.dataSource && [self.dataSource respondsToSelector:@selector(stockDatasWithIndex:)])
        {
            ...
            if(type == Y_StockChartcenterViewTypeOther)
            {
                ...
                NSLog(@"scrollview的偏移量%f和scrollview的contentsize%f",self.kLineView.scrollView.contentOffset.x,self.kLineView.scrollView.contentSize.width);
//                CGPoint contentOffsetPoint = self.kLineView.scrollView.contentOffset;
//                CGRect frame = self.kLineView.scrollView.frame;
//                if (contentOffsetPoint.x == self.kLineView.scrollView.contentSize.width - frame.size.width) {
                    [self.kLineView reDraw];
//                }
            }
            [self bringSubviewToFront:self.segmentView];
